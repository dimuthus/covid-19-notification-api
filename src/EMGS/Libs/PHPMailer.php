...
use EMGS\Libs\phpmailerException;
...
class PHPMailer {
	...
	protected function smtpSend($header, $body) {
		...
		foreach ( $this->cc as $cc ) {
			...
			$this->doCallback ( $isSent, array (), array (
					$cc [0] 
			), array (), $this->Subject, $body, $this->From );
		}
		foreach ( $this->bcc as $bcc ) {
			...
			$this->doCallback ( $isSent, array (), array (), array (
					$bcc [0] 
	 ...
	 * @throws phpmailerException
	 * @return boolean @TODO Should this not be using __set() magic function?
	 */
